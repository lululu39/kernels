...
@triton.jit
def batched_matmul_fwd_cpu_kernel(
    x_ptr, # [B x M x N]
    y_ptr, # [B x N x P]
    z_ptr, # [B x M x P]
    B,
...
@triton.jit
def batched_matmul_bwd_cpu_kernel_dx(
    y_ptr,
    dx_ptr,
    dz_ptr,
    B,
...
@triton.jit
def batched_matmul_bwd_cpu_kernel_dy(
    x_ptr,
    dy_ptr,
    dz_ptr,
    B,
...
def batched_matmul_fwd_cpu(
    ...
    batched_matmul_fwd_cpu_kernel[grid](
        x_ptr=x,
        y_ptr=y,
        z_ptr=z,
        B=B,
...
def batched_matmul_bwd_cpu(
    ...
    batched_matmul_bwd_cpu_kernel_dx[grid](
        y_ptr=y,
        dx_ptr=dx,
        dz_ptr=dz,
        B=B,
    ...
    batched_matmul_bwd_cpu_kernel_dy[grid](
        x_ptr=x,
        dy_ptr=dy,
        dz_ptr=dz,
        B=B,
...
@torch.compile
class BatchedMatmulFunctionCPU(torch.autograd.Function):
    ...
    def backward(ctx, dz):
        ...
        dx, dy = batched_matmul_bwd_cpu(x, y, dz)
...
def batched_matmul_cpu(
    x: torch.Tensor,
    y: torch.Tensor,
) -> torch.Tensor:
    return BatchedMatmulFunctionCPU.apply(x, y)