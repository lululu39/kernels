...
def matmul_bwd_kernel_dx(
    ...
    for p in tl.range(0, P, BP):
        ...
        y = tl.load(y_ptr + range_y, mask_y) # [BB, BN, BP], we transpose it later using tl.trans
...
def matmul_fwd(
    ...
    z = torch.empty(B, M, P, dtype=x.dtype, device=x.device)
