...
# NOTE: tl.dot requires the same dtype
# NOTE: we also partition hidden dimension V, so be careful with the offsets
# NOTE: partition along V is not accurate for backward! so NV should always be 1 in bwd!
...
def flash_attention_2_fwd_kernel(
    ...
    p_o = tl.make_block_ptr(o + bos * V, (T, V), (V, 1), (i_t * BT, i_v * BV), (BT, BV), (1, 0))
    ...
    for i_s in tl.range(0, i_t * BT, BS):
        ...
        p_v = tl.make_block_ptr(v + bos * V, (T, V), (V, 1), (i_s, i_v * BV), (BS, BV), (1, 0))
...
def flash_attention_2_bwd_kernel_dq(
    ...
    p_do = tl.make_block_ptr(do + bos * V, (T, V), (V, 1), (i_t * BT, i_v * BV), (BT, BV), (1, 0))
    ...
    for i_s in tl.range(0, i_t * BT, BS):
        ...
        p_v = tl.make_block_ptr(v + bos * V, (V, T), (1, V), (i_v * BV, i_s), (BV, BS), (0, 1)) # NOTE: v^T
        ...
        b_dp = tl.dot(b_do, b_v) # dp = do @ v^T [BT, BS] NOTE: why cast?
...
def flash_attention_2_bwd_kernel_dkv(
    ...
    p_v = tl.make_block_ptr(v + bos * V, (T, V), (V, 1), (i_t * BT, i_v * BV), (BT, BV), (1, 0))
    p_dk = tl.make_block_ptr(dk + bos * K, (T, K), (K, 1), (i_t * BT, 0), (BT, BK), (1, 0))
    p_dv = tl.make_block_ptr(dv + bos * V, (T, V), (V, 1), (i_t * BT, i_v * BV), (BT, BV), (1, 0))
    ...
    for i_s in tl.range(i_t * BT if causal else 0, min((i_t + 1) * BT, T), BS):
        ...
        p_do = tl.make_block_ptr(do + bos * V, (T, V), (V, 1), (i_s, i_v * BV), (BS, BV), (1,0))
    ...
    # NOTE: inside kernel we use tl.cdiv not triton.cdiv
    for i_s in tl.range((i_t + 1) * BT, tl.cdiv(T, BS) * BS, BS):
...
def flash_attention_2_bwd_preprocess_kernel(
    ...
    b_do = tl.load(do + i_n * V + o_delta, mask=m_delta, other=0).to(tl.float32) # NOTE: casting
...
def flash_attention_2_bwd_preprocess(
    ...
    B, H, T, V, = o.shape
    delta = torch.empty(B, H, T, dtype=torch.float, device=o.device)
...
def flash_attention_2_bwd(
    ...
    # NOTE: coppied from fla
    # NOTE: bwd is not the same as fwd
    # we need both NK and NV to be 1 to avoid atomic addition, especially for NV
